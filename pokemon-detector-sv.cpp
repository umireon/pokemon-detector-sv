...
static constexpr std::array<int, 2> convertInt2ToStdArray2(const int cArray[2])
{
...
convertInt62ToVector6Array2(const int cArray[6][2])
{
	return std::vector<std::array<int, 2>>{
		convertInt2ToStdArray2(cArray[0]),
		convertInt2ToStdArray2(cArray[1]),
		convertInt2ToStdArray2(cArray[2]),
		convertInt2ToStdArray2(cArray[3]),
		convertInt2ToStdArray2(cArray[4]),
		convertInt2ToStdArray2(cArray[5]),
	};
}

extern "C" struct pokemon_detector_sv_context {
	pokemon_detector_sv_context(
		const struct pokemon_detector_sv_config config)
		: config(config),
		  sceneDetector(config.classifier_lobby_my_select,
		  ...
		  opponentPokemonCropper(
			  ...
			  convertInt62ToVector6Array2(
				  config.opponent_row_range)),
		  opponentPokemonIds(6),
		  selectionOrderCropper(
			  convertInt2ToStdArray2(
				  config.selection_order_range_col),
			  convertInt62ToVector6Array2(
				  config.selection_order_range_row)),
		  resultCropper(convertInt2ToStdArray2(config.result_range_col),
				std::vector<std::array<int, 2>>{
					convertInt2ToStdArray2(
						config.result_range_row[0])}),
		  resultRecognizer(config.result_n_bins,
				   config.result_lose_max_index,
				   config.result_lose_ratio,
